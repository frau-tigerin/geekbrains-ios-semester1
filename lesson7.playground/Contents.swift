...
enum CarError: Error {
    case FuelCapacityCanNotBeBelowZero
}
...
class Car {
    ...
    func apply(action: Action) throws {
        ...
        case .spendFuel(let capacity):
            guard fuel >= capacity else {
                throw CarError.FuelCapacityCanNotBeBelowZero
            }
            fuel -= capacity
        default:
            break
...
class SportCar: Car, CustomStringConvertible {
    ...
    override func apply(action: Action) throws {
        switch action {
...
print(car1)
try car1.apply(action: .loadGoodsToTrunk(goodsVolume: 10))
print(car1)

do {
    try car1.apply(action: .spendFuel(capacity: 200))
} catch {
    print(error)
}
print(car1)
...
print(car2)
try car2.apply(action: .closeWindow)
print(car2)
